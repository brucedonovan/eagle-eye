...
| `GET` | `/catalog/cached` | List disk-cached catalog courses (no golfapi.io call) |
...
**Discovery** talks to a **course catalog** interface (`CourseCatalogProvider`), not a vendor SDK. Search and course identity use [golfapi.io](https://golfapi.io/) only (`COURSE_CATALOG_PROVIDER=golfapi`): search is clubs (`GET /clubs`); a club owns nested courses; selecting one fetches scorecard + GPS (`GET /courses/{id}`, `GET /coordinates/{id}`). Course/GPS payloads are **cached on disk forever**; club search is cached for 30 days. Another vendor with clubs, courses, scorecards, and GPS is a drop-in: implement `providers/<name>.py`, `register_provider(...)`, set `COURSE_CATALOG_PROVIDER`. Overpass still supplies `leisure=golf_course` and `golf=*` polygons after a catalog hit. Catalog `/coordinates` then overlay pins, tees, hole numbers, dogleg centerlines, and bunker/water seeds onto those OSM layers.

**Imagery** is a ranked registry (Nearmap / Maxar / Planet / Google / Mapbox when keys exist, otherwise ESRI World Imagery, then Sentinel-2 / USGS). The course mosaic stays on public ESRI. A second mosaic (EOX cloudless / ESRI Clarity) and a temporal mosaic are fetched when the tile budget allows, plus z19 pin crops (Google Static or Mapbox when keyed, else ESRI).

**Segmentation** votes 2-of-3 mosaics, with OSM polygons and golfapi GPS as priors. Catalog seeds may add missing bunkers/water from the primary mosaic; unseeded imagery adds still need two sources to agree. Greens may shrink to a pin-seeded imagery outline. Optional `SEGMENTATION_WEIGHTS` (ONNX/SAM) are a refine hook and are never required. No GPU for the default path. Limits of this pass: RGB-only (no NIR), tree/building shadows, and seasonal/dormant grass that looks like sand or rough.
