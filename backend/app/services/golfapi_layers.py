"""Fuse Golf API GPS / scorecard into OSM layers.

Golf API takes preference for identity (hole numbers, pins, tee boxes that
OSM is missing) and for the working AOI when its GPS hull is tighter than
the OSM club boundary. OSM keeps polygon geometry (greens, bunkers, fairways).
"""
...
def fuse_layers(
    ...
    *,
    ...
    stats["pins_golfapi"] = len(pin_pts)
    layers["pin"] = _merge_pins(layers.get("pin"), pin_pts, stats)
    stats["greens_numbered"] = _stamp_nearest(layers.get("green"), pin_pts, max_m=45.0)
    stats["tees_added"] = _merge_tees(layers, tee_pts)
    ...
    if osm_holes:
        ...
        lines = _centerlines_from_gps(pin_pts, tee_pts, scorecard)
        if lines:
...
def _merge_pins(existing: dict[str, Any] | None, golfapi_pins: list[dict[str, Any]], stats: dict[str, int]) -> dict:
    feats = []
    used_osm: set[int] = set()
    osm_feats = list((existing or {}).get("features") or [])
    for point in golfapi_pins:
        props = {
            "source": "golfapi",
            "kind": point["kind"],
            "hole": point.get("hole"),
    ...
    for i, feat in enumerate(osm_feats):
        ...
        if any(distance_meters(pt, Point(p["lon"], p["lat"])) <= PIN_REPLACE_M for p in golfapi_pins):
            continue
...
def _stamp_nearest(fc: dict[str, Any] | None, pins: list[dict[str, Any]], max_m: float) -> int:
    ...
    for feat in fc.get("features") or []:
        ...
        if best and best.get("hole"):
            ...
            props["golfapi_hole"] = best["hole"]
            stamped += 1
    return stamped


def _merge_tees(layers: dict[str, dict[str, Any]], tee_pts: list[dict[str, Any]]) -> int:
        ...
        existing.append(
            to_feature(
                poly,
                {
                    "source": "golfapi",
                    "hole": point.get("hole"),
                    "instance_id": f"golfapi-tee-{point.get('hole') or added}-{added}",
                },
            )
        )
        added += 1
