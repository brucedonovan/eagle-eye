...
from app.services import golfapi, golfapi_layers
...
def test_clubs_own_courses():
    hits = golfapi.flatten_club_search(PEBBLE_CLUBS)
    assert len(hits) == 2
...
async def test_search_cache_avoids_http(tmp_path):
    client = golfapi.GolfAPIClient(cache_dir=tmp_path, api_key="test-key", search_ttl_days=30)
    calls = {"n": 0}
...
def test_fuse_prefers_golfapi_pins():
    ...
    stats = golfapi_layers.fuse_layers(layers, points, scorecard={"pars_men": [4], "indexes_men": [6]})
    pins = layers["pin"]["features"]
    sources = [f["properties"]["source"] for f in pins]
    assert sources.count("golfapi") == 1
    assert "openstreetmap" in sources
    assert stats["pins_golfapi"] == 1
    assert layers["green"]["features"][0]["properties"]["hole"] == 1
    assert layers["hole_centerline"]["features"][0]["properties"]["par"] == 4


async def test_discovery_uses_cached_golfapi(work_dir, monkeypatch):
    from app.config import settings

    monkeypatch.setattr(settings, "golfapi_key", "test-key")

    async def fake_bundle(self, course_id, timestamp_updated=None):
        return {
            "course": {
                "courseID": course_id,
                "clubID": "club1",
            ...
            "course_cached": True,
            "coordinates_cached": True,
            "api_requests_left": "99",
        }

    async def no_osm(lon, lat, name, pad_deg=0.03):
        return []

    monkeypatch.setattr(golfapi.GolfAPIClient, "load_course_bundle", fake_bundle)
    monkeypatch.setattr("app.services.overpass.find_golf_courses", no_osm)

    ctx = PipelineContext(
        job_id="golfapi-job",
        work_dir=work_dir,
        request={"golfapi_course_id": "012141520658891108829"},
    )
    await discovery.run(ctx)
    assert ctx.course["source"] == "golfapi"
    assert ctx.course["golfapi_cached"] is True
